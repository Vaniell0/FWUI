/*
 * fwui_native.c — C extension for FWUI HTML rendering.
 *
 * Three rendering modes:
 *   1. to_html_native — walks FWUI::Node tree, renders HTML
...
static VALUE fwui_native_to_html(VALUE self) {
...
void Init_fwui_native(void) {
    ...
    VALUE cNode = rb_const_get(mFWUI, rb_intern("Node"));

    detect_ivar_layout(cNode);

    rb_define_method(cNode, "to_html_native", fwui_native_to_html, 0);

    /* Baked templates module methods */
    VALUE mNative = rb_define_module_under(mFWUI, "NativeBaked");
    ...
    static const rb_data_type_t baked_gc_type = {
        .wrap_struct_name = "fwui_baked_gc",
