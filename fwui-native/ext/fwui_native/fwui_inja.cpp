/*
 * fwui_inja.cpp — Inja template engine bindings for fwui-native.
 *
 * Provides:
 *   FWUI::Native.render_template(template_str, data_hash) → String
 *   FWUI::Native.render_template_file(path, data_hash)    → String
...
/* ── Init (called from Init_fwui_native in fwui_native.c) ─────────── */

extern "C" void Init_fwui_inja(void) {
    inja_enc_utf8 = rb_utf8_encoding();

    VALUE mFWUI = rb_const_get(rb_cObject, rb_intern("FWUI"));
    VALUE mNative = rb_define_module_under(mFWUI, "NativeInja");
