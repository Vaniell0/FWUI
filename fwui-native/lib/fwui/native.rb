...
# Load the pure Ruby fwui gem first (runtime dependency)
require 'fwui'

# Load the compiled C extension
require 'fwui_native/fwui_native'

# Patch FWUI::Node: C version becomes the default to_html
...
module FWUI
  ...
  module Native
    SLOT_RE = /__FWUI_SLOT_(\w+)__/

    # Compile a template Node into a baked template.
    #   FWUI::Native.bake(:card, FWUI.div([FWUI.slot(:title)]))
    ...
    #   FWUI::Native.render_baked(:card, title: "Hello")
    # Returns HTML String.
    ...
    def self.baked_node(name, **params)
      FWUI.raw(render_baked(name, **params))
    end

    # Build HTML via PackedBuilder opcode stream — no Node objects.
    #   FWUI::Native.build { div { h1("Title").bold } }
    ...
    #   FWUI::Native.render_template_file("templates/page.html", { title: "Home" })
    ...
    # Check if Inja is available (always true when fwui-native is loaded).
