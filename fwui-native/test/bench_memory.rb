...
# Memory benchmark for FWUI Ruby + Native rendering.
#
...
module BenchMemory
  ...
  def self.build_widget(id, value)
    FWUI.div([
      FWUI.h3("Widget #{id}").bold.color("#1a1a2e"),
      FWUI.div([
        FWUI.span_elem("#{value}").font_size("32px").bold.color("#0f3460"),
        FWUI.span_elem("units").font_size("14px").color("#888"),
      ]).set_style("display", "flex").set_style("align-items", "baseline").set_style("gap", "8px"),
      FWUI.div(
        (1..8).map { |j|
          FWUI.div.set_style("width", "12%")
      ...
      FWUI.p("Last updated: tick 0").color("#999").font_size("12px"),
    ]).add_class("widget")
  ...
  def self.build_dashboard(n)
    ...
    FWUI.div([
      FWUI.header([
        FWUI.h1("Operations Dashboard").color("#1a1a2e"),
        FWUI.p("Real-time monitoring — #{n} widgets").color("#666"),
      ...
      FWUI.footer([
        FWUI.p("FWUI Dashboard v1.0").color("#999").font_size("12px"),
  ...
  def self.run
    puts "=" * 72
    puts "FWUI Memory Benchmark — Ruby Native (subtree cache)"
