...
# Tests for fwui-native C extension.
...
module NativeTests
  ...
  def self.run
    puts "=== fwui-native C extension tests ==="
  ...
  def self.test_basic_elements
    puts "  basic elements..."
    match("empty div", FWUI.div)
    match("div with text", FWUI.div.set_text("Hello"))
    match("div with id", FWUI.div.set_id("main"))
    match("div with one class", FWUI.div.add_class("active"))
    match("div with two classes", FWUI.div.add_class("a").add_class("b"))
    match("p with text", FWUI.p("Hello world"))
    match("h1", FWUI.h1("Title"))
    match("span", FWUI.span_elem("inline"))
  end

  def self.test_escaping
    puts "  HTML escaping..."
    match("escape &", FWUI.p("A & B"))
    match("escape <", FWUI.p("A < B"))
    match("escape >", FWUI.p("A > B"))
    match("escape \"", FWUI.div.set_id('say "hi"'))
    match("escape '", FWUI.p("it's"))
    match("escape mixed", FWUI.p('<script>alert("xss")</script>'))
    match("escape in attr value", FWUI.a("link", href: "/search?q=a&b=c"))
    match("escape in class", FWUI.div.add_class("a&b"))
  end

  def self.test_children
    puts "  children..."
    match("two children",
      FWUI.div([FWUI.h1("Title"), FWUI.p("Body")]))
    match("nested 3 levels",
      FWUI.div([FWUI.section([FWUI.article([FWUI.p("Deep")])])]))
    match("mixed text + children",
      FWUI.div([FWUI.text("Before"), FWUI.p("Mid"), FWUI.text("After")]))
    match("many children",
      FWUI.ul((1..10).map { |i| FWUI.li("Item #{i}") }))
  end

  def self.test_special_nodes
    puts "  special nodes..."
    match("text node", FWUI.text("Hello <world>"))
    match("raw node", FWUI.raw("<b>Bold</b>"))
    match("raw in tree", FWUI.div([FWUI.raw("<hr class='sep'>")]))
    match("empty text node", FWUI.text(""))
  end
  ...
  def self.test_void_elements
    ...
    match("br", FWUI.br)
    match("hr", FWUI.hr)
    match("img", FWUI.img("photo.jpg", alt: "A photo"))
    match("input", FWUI.input_elem("text", name: "q", value: "search"))
    match("meta", FWUI.meta_elem(charset: "UTF-8"))
    match("link", FWUI.link_elem(rel: "stylesheet", href: "/style.css"))
  end

  def self.test_styles_and_attrs
    puts "  styles and attributes..."
    match("one style",
      FWUI.div.set_style("color", "red"))
    match("multiple styles",
      FWUI.div.set_style("color", "red").set_style("margin", "10px").set_style("padding", "5px"))
    match("one attr",
      FWUI.div.set_attr("data-page", "home"))
    match("multiple attrs",
      FWUI.div.set_attr("data-a", "1").set_attr("data-b", "2").set_attr("role", "main"))
    match("id + class + style + attr",
      FWUI.div.set_id("x").add_class("y").set_style("color", "red").set_attr("data-z", "1"))
  end

  def self.test_utf8
    puts "  UTF-8..."
    match("cyrillic", FWUI.p("Привет мир"))
    match("japanese", FWUI.p("日本語テスト"))
    match("emoji", FWUI.p("Hello 🎉🚀"))
    match("mixed utf8", FWUI.p("Привет 日本語 🎉"))
    match("utf8 in attr", FWUI.div.set_attr("data-name", "Иван"))
    match("utf8 in id", FWUI.div.set_id("блок"))
  end

  def self.test_document
    puts "  document..."
    match("full document",
      FWUI.document("Test Page", body: [FWUI.h1("Hello"), FWUI.p("World")]))
    match("document with head",
      FWUI.document("Page", head: [FWUI.style_elem("body{margin:0}")], body: [FWUI.p("Hi")]))
  end

  def self.test_decorator_chains
    puts "  decorator chains..."
    match("bold + color",
      FWUI.h1("Title").bold.color("#333"))
    match("full chain",
      FWUI.p("Text").bold.italic.color("red").bg_color("yellow").padding("10px").margin("5px"))
    match("pipe operator",
      FWUI.p("Hello") | FWUI.Bold() | FWUI.Color("blue"))
  end

  def self.test_large_tree
    puts "  large tree (500 nodes) byte-for-byte..."
    tree = FWUI.div((1..500).map { |i|
      FWUI.div([
        FWUI.h1("Item #{i}").bold.color("#333"),
        FWUI.p("Desc #{i}").padding("10px").margin("5px"),
        FWUI.a("Link", href: "/#{i}"),
      ]).add_class("card")
    })
  ...
  def self.test_render_cache
    puts "  render cache..."
    node = FWUI.div([FWUI.h1("Hello").bold, FWUI.p("World")])
  ...
  def self.test_subtree_cache
    ...
    child_a = FWUI.h1("Title").bold
    child_b = FWUI.p("Body").color("red")
    root = FWUI.div([child_a, child_b]).add_class("box")
  ...
  def self.test_baked
    ...
    baked_html = FWUI::Native.render_baked(:greeting, title: "Hello", body: "World")
    manual_html = FWUI.div([
      FWUI.h1("Hello").bold,
      FWUI.p("World"),
    ...
    escaped = FWUI::Native.render_baked(:greeting, title: "<script>", body: "A & B")
    assert("baked escapes title") { escaped.include?("&lt;script&gt;") }
    assert("baked escapes body") { escaped.include?("A &amp; B") }

    # Attribute slots
    FWUI::Native.bake(:link,
      FWUI.a(FWUI.slot(:text), href: FWUI.slot_attr(:url))
    )
    link_html = FWUI::Native.render_baked(:link, text: "Click", url: "/page?a=1&b=2")
    ...
    node = FWUI::Native.baked_node(:greeting, title: "Hi", body: "There")
    html = node.to_html
    assert("baked_node renders") { html.include?("Hi") && html.include?("There") }

    # Multi-slot template
    FWUI::Native.bake(:card,
      FWUI.div([
        FWUI.h1(FWUI.slot(:title)).bold.color("#333"),
        FWUI.p(FWUI.slot(:desc)),
        FWUI.a("Link", href: FWUI.slot_attr(:link)),
      ]).add_class("card")
    )
    card_html = FWUI::Native.render_baked(:card, title: "Project", desc: "Cool stuff", link: "/p/1")
  ...
  def self.test_packed_builder
    ...
    assert_eq("packed: empty div",
      FWUI::Native.build { div },
      "<div></div>")

    assert_eq("packed: div with text",
      FWUI::Native.build { div("Hello") },
      "<div>Hello</div>")

    assert_eq("packed: h1 with text",
      FWUI::Native.build { h1("Title") },
      "<h1>Title</h1>")

    assert_eq("packed: p with text",
      FWUI::Native.build { p("Body") },
      "<p>Body</p>")

    # Attributes
    assert_eq("packed: div with id",
      FWUI::Native.build { div.set_id("main") },
      '<div id="main"></div>')

    assert_eq("packed: div with class",
      FWUI::Native.build { div.add_class("active") },
      '<div class="active"></div>')

    assert_eq("packed: div with two classes",
      FWUI::Native.build { div.add_class("a").add_class("b") },
      '<div class="a b"></div>')

    assert_eq("packed: div with attr",
      FWUI::Native.build { div.set_attr("data-page", "home") },
      '<div data-page="home"></div>')

    # Styles
    assert_eq("packed: bold style",
      FWUI::Native.build { h1("Title").bold },
      '<h1 style="font-weight: bold">Title</h1>')

    assert_eq("packed: multiple styles",
      FWUI::Native.build { div.set_style("color", "red").set_style("margin", "10px") },
    ...
    assert_eq("packed: text escaping",
      FWUI::Native.build { p("A & B < C > D") },
      "<p>A &amp; B &lt; C &gt; D</p>")

    assert_eq("packed: attr escaping",
      FWUI::Native.build { a("link", href: "/search?q=a&b=c") },
      '<a href="/search?q=a&amp;b=c" target="_self">link</a>')

    # Text and raw nodes
    assert_eq("packed: text node",
      FWUI::Native.build { text("Hello <world>") },
      "Hello &lt;world&gt;")

    assert_eq("packed: raw node",
      FWUI::Native.build { raw("<b>Bold</b>") },
      "<b>Bold</b>")
    ...
    # Void elements
    assert_eq("packed: br",
      FWUI::Native.build { br },
      "<br>")

    assert_eq("packed: hr",
      FWUI::Native.build { hr },
      "<hr>")

    assert_eq("packed: img with attrs",
      FWUI::Native.build { img(src: "photo.jpg", alt: "A photo") },
      '<img src="photo.jpg" alt="A photo">')

    assert_eq("packed: input",
      FWUI::Native.build { input(type: "text", name: "q") },
      '<input type="text" name="q">')

    assert_eq("packed: meta",
      FWUI::Native.build { meta(charset: "UTF-8") },
      '<meta charset="UTF-8">')
    ...
    assert_eq("packed: utf8 text",
      FWUI::Native.build { p("Привет 🎉") },
      "<p>Привет 🎉</p>")

    # Document helper
    doc_html = FWUI::Native.build {
      document("Test Page") {
        h1("Hello")
        p("World")
      }
    ...
    node_html = FWUI.div((1..500).map { |i|
      FWUI.div([
        FWUI.h1("Item #{i}").bold.color("#333"),
        FWUI.p("Desc #{i}").padding("10px").margin("5px"),
        FWUI.a("Link", href: "/#{i}"),
  ...
  def self.test_inja
    ...
    assert_eq("inja: for loop",
      FWUI::Native.render_template("{% for x in items %}{{ x }} {% endfor %}", { items: ["a", "b", "c"] }),
      "a b c ")

    # Conditionals
    assert_eq("inja: if true",
      FWUI::Native.render_template("{% if show %}yes{% else %}no{% endif %}", { show: true }),
      "yes")
    assert_eq("inja: if false",
      FWUI::Native.render_template("{% if show %}yes{% else %}no{% endif %}", { show: false }),
      "no")

    # Nested data
    assert_eq("inja: nested hash",
      FWUI::Native.render_template("{{ user.name }} ({{ user.age }})", { user: { name: "Ivan", age: 25 } }),
      "Ivan (25)")

    # Numeric types
    assert_eq("inja: int and float",
      FWUI::Native.render_template("{{ n }}, {{ f }}", { n: 42, f: 3.14 }),
      "42, 3.14")

    # Empty data
    assert_eq("inja: static text",
      FWUI::Native.render_template("static", {}),
      "static")

    # HTML pass-through (Inja does not escape by default)
    assert_eq("inja: html passthrough",
      FWUI::Native.render_template("{{ c }}", { c: "<b>bold</b>" }),
      "<b>bold</b>")

    # Inja built-in functions
    assert_eq("inja: length()",
      FWUI::Native.render_template("{{ length(items) }}", { items: [1, 2, 3] }),
      "3")

    # inja_available? always true
    assert("inja: available?") { FWUI::Native.inja_available? }
    ...
    assert_eq("inja: render_template_file",
      FWUI::Native.render_template_file(f.path, { title: "Hello" }),
      "<h1>Hello</h1>")
    f.unlink
    ...
    Dir.mktmpdir do |dir|
      ...
      FWUI::Native.set_template_directory(dir + "/")
