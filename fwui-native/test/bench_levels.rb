...
module BenchLevels
  ...
  def self.run
    puts "=" * 60
    puts "FWUI Multi-Level Benchmark (#{N} iterations, 500-node tree)"
    puts "=" * 60
    puts
    ...
    r = bench("Registry cache (raw)") { FWUI.raw(cached_html).to_html }
    results << r
    ...
    FWUI::Native.bake(:bench_card,
      FWUI.div([
        FWUI.h1(FWUI.slot(:title)).bold.color("#333"),
        FWUI.p(FWUI.slot(:desc)).padding("10px").margin("5px"),
        FWUI.a("Link", href: FWUI.slot(:link)),
      ]).add_class("card")
    )

    r = bench("Baked card (1 card)") {
      FWUI::Native.render_baked(:bench_card,
    ...
    r = bench("Baked 500 cards") {
      ...
      FWUI.div(cards.map { |c| FWUI.raw(c) }).to_html_native
    }
    results << r
