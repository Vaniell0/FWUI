...
# FWUI Ruby web server — stdlib Socket, zero dependencies.
#
# Usage:
#   ruby examples/ruby/server.rb            # production
#   ruby examples/ruby/server.rb --dev      # hot-reload + WebSocket
#   FWUI_DEV=1 ruby examples/ruby/server.rb # same

require 'socket'
require 'uri'
require 'json'
require_relative '../../ruby/lib/fwui'

include FWUI

PORT = (ENV['FWUI_PORT'] || 10_101).to_i
PROJECT_ROOT = Platform.gem_root
PAGES_DIR = File.join(PROJECT_ROOT, 'ruby', 'pages')
STATIC_DIR = File.join(PROJECT_ROOT, 'static')

DEV_MODE = ENV['FWUI_DEV'] == '1' || ARGV.include?('--dev')

if DEV_MODE
  require_relative '../lib/fwui/websocket'
  require_relative '../lib/fwui/interactive_hub'
  require_relative '../lib/fwui/file_watcher'
  require_relative '../lib/fwui/hot_reload_js'
end
...
def serve_static(path)
  ...
  canonical = File.realpath(file_path) rescue nil
  static_canonical = File.realpath(STATIC_DIR) rescue STATIC_DIR
  if canonical && canonical.start_with?(static_canonical) && File.file?(canonical)
    ext = File.extname(canonical)
    content_type = MIME_TYPES[ext] || 'application/octet-stream'
    body = File.binread(canonical)
...
if DEV_MODE
  ws_hub = FWUI::InteractiveHub.new
  ...
  puts "[dev] WebSocket endpoint: ws://localhost:#{PORT}/__fwui_ws"
end
...
puts "FWUI server v#{FWUI.version} on http://localhost:#{PORT}"
...
while $running
  ...
  threads << Thread.new(client) do |c|
    ...
    if DEV_MODE && req[:path] == '/__fwui_ws' &&
       req[:headers]['upgrade']&.downcase == 'websocket'
      if FWUI::WebSocket.handshake(c, req[:headers])
    ...
    unless DEV_MODE && req && req[:path] == '/__fwui_ws'
      c.close rescue nil
    end
  end
end
