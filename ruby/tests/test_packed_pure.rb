...
module PackedTests
  ...
  def self.test_dfs_order
    ...
    tree = FWUI.div([
      FWUI.section([FWUI.p("A")]),
      FWUI.article([FWUI.p("B")]),
    ])
  ...
  def self.test_void_element
    puts "  void element..."
    node = FWUI.img("photo.jpg", alt: "pic")
    buf = node.to_packed
    nodes = unpack_all(buf)
    assert_eq("1 node", nodes.size, 1)
  ...
  def self.test_node_count
    ...
    tree = FWUI.div([FWUI.p("A"), FWUI.p("B")])
    buf = tree.to_packed
    count = buf[0, 4].unpack1('V')
  ...
  def self.test_large_tree
    ...
    children = (1..499).map { |i| FWUI.p("Item #{i}") }
    tree = FWUI.div(children)
    buf = tree.to_packed
    nodes = unpack_all(buf)
  ...
  def self.test_document_tree
    puts "  document tree..."
    doc = FWUI.document("Test", body: [FWUI.h1("Hello")])
    buf = doc.to_packed
    nodes = unpack_all(buf)
