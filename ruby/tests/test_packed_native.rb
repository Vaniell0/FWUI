...
# Requires libfwui.so built — tests to_html_packed via C++ FFI.
#
# Run: ruby ruby/tests/test_packed_native.rb

require_relative '../lib/fwui'
...
module PackedNativeTests
  ...
  def self.run
    ...
    begin
      FWUI.send(:_require_native!)
      puts "  libfwui loaded: #{FWUI::Platform.find_library('fwui')}"
    rescue => e
      puts "  SKIP: #{e.message}"
      puts "  Build libfwui.so first: cmake --build build"
      exit 0
    end
  ...
  def self.test_div_with_text
    puts "  div with text..."
    html = FWUI.div.set_text("Hello").to_html_packed
  ...
  def self.test_div_with_id
    puts "  div with id..."
    html = FWUI.div.set_id("main").to_html_packed
    assert_include("id attr", html, 'id="main"')
  end

  def self.test_classes
    puts "  classes..."
    html = FWUI.div.add_class("a").add_class("b").to_html_packed
    assert_include("class attr", html, 'class="a b"')
  end

  def self.test_styles
    puts "  styles..."
    html = FWUI.div.set_style("color", "red").to_html_packed
  ...
  def self.test_attributes
    puts "  attributes..."
    html = FWUI.img("photo.jpg", alt: "pic").to_html_packed
  ...
  def self.test_children
    puts "  children..."
    tree = FWUI.div([FWUI.p("First"), FWUI.p("Second")])
  ...
  def self.test_raw_node
    puts "  raw node..."
    tree = FWUI.div([FWUI.raw("<b>Bold</b>")])
    html = tree.to_html_packed
  ...
  def self.test_text_node_in_tree
    ...
    tree = FWUI.div([FWUI.text("plain text")])
    html = tree.to_html_packed
  ...
  def self.test_matches_pure_ruby
    ...
    tree = FWUI.div([
      ...
      FWUI.p("Description").padding("10px"),
      FWUI.a("Link", href: "/page"),
  ...
  def self.test_complex_tree
    puts "  complex tree..."
    tree = FWUI.div([
      FWUI.h1("Title").bold.color("#333").set_id("title"),
      FWUI.div([
        FWUI.p("Para 1").padding("5px").margin("10px"),
        FWUI.p("Para 2").add_class("highlight"),
        FWUI.img("logo.png", alt: "Logo"),
        FWUI.raw("<hr class='sep'>"),
      ]).add_class("content"),
      FWUI.footer([
        FWUI.a("Home", href: "/").add_class("nav-link"),
        FWUI.text(" | "),
        FWUI.a("About", href: "/about"),
      ]),
    ]).set_id("page")
  ...
  def self.test_document
    puts "  document..."
    doc = FWUI.document("Test Page", body: [
      FWUI.h1("Welcome"),
      FWUI.p("Content"),
    ])
  ...
  def self.test_large_tree_perf
    ...
    tree = FWUI.div(children).set_id("list")
