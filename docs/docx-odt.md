...
[← Главная](index.md) · [fwui-native](fwui-native.md) · [pdf](pdf.md)
...
```ruby
FWUI::Native.to_docx(node)
FWUI::Native.to_docx(node, "page_size" => "A4")
FWUI::Native.to_odt(node)
FWUI::Native.to_odt(node, "page_size" => "A4")
```

### FWUI::Doc — документ-ориентированный API

```ruby
FWUI::Doc.to_docx(node, preset: :gost)
FWUI::Doc.to_docx(node, preset: :gost, header: "Мой отчёт")
FWUI::Doc.to_odt(node, preset: :gost, page_numbers: true)
```
...
File.binwrite("coursework.docx", FWUI::Doc.to_docx(doc,
  preset: :gost,
  header: "Курсовая работа",
  page_numbers: true,
  first_page_footer: "Севастополь, 2026",
...
```ruby
FWUI.page_break          # в основном DSL
FWUI::Doc.page_break     # через Doc namespace
```
...
```ruby
FWUI.p("Новая страница").set_style("page-break-before", "always")
```
...
```ruby
FWUI.node("td", text: "Wide", colspan: "2")    # colspan → w:gridSpan / table:number-columns-spanned
FWUI.node("td", text: "Tall", rowspan: "3")    # rowspan → w:vMerge / table:number-rows-spanned
```
